...
def detikcom_url(query, category_id, date_start, date_end, page_num=1, timeout=20, sleep_s=0.8):
    url = (
...
def last_page_article_count(query, category_id, date_start, date_end, page_num, timeout=20, sleep_s=0.8):
...
def detikcom_search_results(query, category_id, date_start, date_end, timeout=20, sleep_s=0.8):
    page = detikcom_url(
        query=query,
        category_id=category_id,
        date_start=date_start,
...
def detikcom_get_content(article_url, timeout=20, sleep_s=0.8):
    ...
    try:
        time.sleep(sleep_s)
        response = requests.get(article_url, headers=DEFAULT_HEADERS, timeout=timeout)
        response.raise_for_status()
        ...
        if multiple_page:
            ...
            for page_url in multiple_urls:
                time.sleep(sleep_s)
                page_response = requests.get(page_url, headers=DEFAULT_HEADERS, timeout=timeout)
...
def detikcom_advertorial_check(article_url, timeout=20, sleep_s=0.8):
    try:
        time.sleep(sleep_s)
        response = requests.get(article_url, headers=DEFAULT_HEADERS, timeout=timeout)
        response.raise_for_status()
